				...
				if (!scheme.hasOwnProperty("isShown") || scheme.isShown) {
					...
					console.log(
						chalk.gray(
							` ${chalk.green(
								"✔"
							)} [hide-library-schemes]: ${path.dirname(
...
module.exports = function findAndFix() {
	...
	const userSpecificPattern =
		"../../node_modules/**/*.xcodeproj/xcuserdata/*.xcuserdatad/xcschemes/xcschememanagement.plist";

	console.log(chalk.gray("Hiding schemes from node_modules xcode projects."));

	utilities.updatePlistsMatchingGlob(
		userSpecificPattern,
		(err, file, filePath) => {
