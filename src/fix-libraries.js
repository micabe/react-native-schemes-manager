...
function configListForConfig(configLists, configUuid) {
	for (const listUuid of Object.keys(configLists)) {
		...
		if (
			configList.buildConfigurations.find(
				config => config.value === configUuid
			)
		) {
			return configList;
		}
...
function updateProject(project) {
	...
	for (const sourceBuildConfig of Object.keys(mappings)) {
		for (const destinationBuildConfig of mappings[sourceBuildConfig]) {
			...
			const buildConfig = project.getBuildConfigByName(
				destinationBuildConfig
			);
			...
			if (Object.keys(buildConfig).length === 0) {
				...
				const sourceConfigs = project.getBuildConfigByName(
					sourceBuildConfig
				);
				...
				for (const key of Object.keys(sourceConfigs)) {
					...
					if (!configList)
						throw new Error(
							`Unable to find config list for build configuration: ${sourceConfig.name}`
						);
					...
					configList.buildConfigurations.push({
						value: configurationUuid,
						comment: destinationBuildConfig
					});
				}

				console.log(
					chalk.gray(
						` ${chalk.green("✔")} [fix-libraries]: ${chalk.green(
							sourceBuildConfig +
								" -> " +
								destinationBuildConfig +
								" created"
				...
				console.log(
					chalk.gray(
						` - [fix-libraries]: ${sourceBuildConfig} -> ${destinationBuildConfig} skipped in ${path.dirname(
							path.relative(process.cwd(), project.filepath)
						)}`
					)
				);
